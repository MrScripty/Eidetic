...
- `ui/src/lib/components/editor/BeatEditor.svelte` is already over the component decomposition threshold. The child-reference pass-through was intentionally minimal; split this editor before adding proposal review controls or additional planning UI.
- `crates/server/src/routes/ai.rs` is over the 500-line decomposition threshold and was touched while making child-plan previews durable. Split AI generation, child planning, context preview, config/status, and batch generation route handlers before adding more AI workflow behavior.
- Child-plan application still does not consume or mark the durable child-plan ID. Thread the plan ID into apply-children acceptance, capture the selected parent before async generation in `BeatEditor`, validate the generated plan belongs to that parent, and mark the plan applied/rejected through backend-owned history instead of losing the preview identity.
...
- Remaining: apply-children should consume the durable child-plan ID, validate parent identity, and mark the plan applied/rejected through history instead of treating the preview identity as disposable metadata.
