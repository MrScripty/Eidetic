...
- Remaining: generate-children previews stay non-durable until the user applies the edited plan, review actions still need accept/reject commands, accepted proposals must compose atomically with bible graph commands, and graph-backed AI context projections are still needed before proposal review can drive propagation.
