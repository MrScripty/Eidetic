...
- Partially resolved: AI context preview now reads persisted timeline and story arc state from SQLite instead of the in-memory project mirror. AI generation, child generation, batch generation, and scene recap still read or mutate `Project.timeline` directly and need the same SQLite/proposal-backed treatment before command-route mirror mutation can be removed.
