...
- Partially resolved: node-range, node-lock, and node-notes commands no longer mutate the in-memory timeline mirror and validate against SQLite project state. The remaining structural timeline command routes still validate against and mutate the mirror; migrate them command-by-command to SQLite project inputs.
