...
- Partially resolved: story arc commands now validate against SQLite, record command/event/revision rows and SQLite current-state rows transactionally, story arc list/progression projections read arcs from SQLite, arc command routes no longer trigger broad project saves, AI prompt construction hydrates arcs from SQLite before calling core prompt builders, and broad project save/autosave preserves SQLite arc current state instead of overwriting it from stale `Project.arcs`. They still refresh `Project.arcs` as a transitional compatibility mirror; remove the remaining in-memory arc mirror before claiming complete backend-owned arc state.
