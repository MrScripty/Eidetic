...
- Partially resolved: story arc commands now validate against SQLite, record command/event/revision rows and SQLite current-state rows transactionally, story arc list/progression projections read arcs from SQLite, and arc command routes no longer trigger broad project saves. They still refresh `Project.arcs` as a transitional compatibility mirror; remove the remaining in-memory arc mirror and route AI/core arc consumers through projections before claiming complete backend-owned arc state.
...
- Partially resolved: bible graph and story arc command/projection DTOs were split out of `ui/src/lib/types.ts` into focused modules, and legacy entity DTOs were removed from the frontend type surface. `types.ts` still mixes timeline, projection primitives, script, AI, diffusion, model, websocket, and helper contracts; continue splitting those before adding semantic proposal or additional Bevy bridge DTOs.
