...
- Partially resolved: bible graph, story arc, AI runtime, websocket, project/reference, and child planning DTOs were split out of `ui/src/lib/types.ts` into focused modules, and legacy entity DTOs were removed from the frontend type surface. `types.ts` still mixes timeline, projection primitives, and helper contracts; continue splitting those before adding semantic proposal or additional Bevy bridge DTOs.
