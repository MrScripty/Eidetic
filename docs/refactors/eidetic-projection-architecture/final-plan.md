...
- Partially resolved: story arc commands now record command/event/revision rows and SQLite current-state rows transactionally, story arc list/progression projections read arcs from SQLite, and arc command routes no longer trigger broad project saves. They still mirror into `Project.arcs` for transitional compatibility; remove the remaining in-memory arc mirror and route AI/core arc consumers through projections before claiming complete backend-owned arc state.
