...
- Partially resolved: timeline structural commands now record sparse SQLite command/event/revision rows with replay protection across node range, node create/delete/split, child replacement, node lock, node notes, and relationship create/delete commands, while still updating backend-owned in-memory project state and triggering the existing save path. Move timeline projections and persistence to canonical SQLite current-state tables before claiming complete backend-owned timeline state.
- Partially resolved: story arc commands now validate against SQLite, record command/event/revision rows and SQLite current-state rows transactionally, story arc list/progression projections read arcs from SQLite, arc command routes no longer trigger broad project saves, and AI prompt construction hydrates arcs from SQLite before calling core prompt builders. They still refresh `Project.arcs` as a transitional compatibility mirror; remove the remaining in-memory arc mirror and update save/autosave so broad project serialization cannot overwrite SQLite arcs before claiming complete backend-owned arc state.
- Child planning proposals still contain character, location, and prop references, but the new timeline apply-children command intentionally does not mutate bible graph state. A graph-backed AI context projection and semantic/bible proposal command path are still needed to turn those references into reviewed bible graph updates.
...
- Partially resolved: bible graph, story arc, projection primitives, AI runtime, websocket, project/reference, and child planning DTOs were split out of `ui/src/lib/types.ts` into focused modules, and legacy entity DTOs were removed from the frontend type surface. `types.ts` still mixes timeline models, UI constants, and helper contracts; continue splitting those before adding semantic proposal or additional Bevy bridge DTOs.
