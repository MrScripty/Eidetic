...
- Partially resolved: node-range, node-lock, node-notes, split-node, delete-node, and relationship create/delete commands no longer mutate the in-memory timeline mirror and validate against SQLite project state. The remaining create-node/apply-children command routes still validate against and mutate the mirror; migrate them command-by-command to SQLite project inputs.
