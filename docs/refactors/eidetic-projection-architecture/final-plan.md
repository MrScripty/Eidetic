...
- Story arc commands now own the HTTP mutation surface, but they still mutate `Project.arcs` and rely on the full project save path rather than writing sparse SQLite command/event/revision rows. Migrate arc create/update/delete to the history-backed storage model before claiming durable undo/redo, replay, or per-field traceability for arcs.
