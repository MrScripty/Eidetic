...
- Partially resolved: timeline structural commands currently update backend-owned in-memory project state and trigger the existing save path. The node range, node lock, node notes, and relationship create commands now record sparse SQLite command/event/revision rows with replay protection; node create/delete, split, child replacement, and relationship delete still need the same history-backed treatment before claiming durable undo/redo or replay for all timeline edits.
