...
- Remaining timeline ownership cleanup: timeline command routes still mutate the in-memory project mirror after SQLite writes for transitional non-projection side effects. Remove that mirror mutation path once AI/Y.Doc/save call sites no longer rely on `Project.timeline` as a compatibility cache.
- Partially resolved: AI context preview, generation request construction, child generation request construction, and batch child discovery now read persisted timeline and story arc state from SQLite instead of the in-memory project mirror. AI generation status updates and generated-content status updates now persist to SQLite current state before transitional mirror updates. Scene recap still mutates/reads `Project.timeline` directly and needs SQLite/proposal-backed treatment before command-route mirror mutation can be removed.
