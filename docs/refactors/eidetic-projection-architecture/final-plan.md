...
- Partially resolved: node-range, node-lock, node-notes, create-node, split-node, delete-node, and relationship create/delete commands no longer mutate the in-memory timeline mirror and validate against SQLite project state. The remaining apply-children command route still validates against and mutates the mirror; migrate it to SQLite project inputs.
