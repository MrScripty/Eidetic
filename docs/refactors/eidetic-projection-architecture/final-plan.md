...
- Remaining: generate-children previews stay non-durable until the user applies the edited plan, proposal acceptance still needs a command that composes atomically with bible graph mutations, and graph-backed AI context projections are still needed before proposal review can drive propagation.
