...
- Partially resolved: story arc commands now record command/event/revision rows and SQLite current-state rows transactionally, and story arc list/progression projections read arcs from SQLite. They still mirror into `Project.arcs` and trigger the full project save path for transitional compatibility; remove the remaining in-memory arc mirror and route AI/core arc consumers through projections before claiming complete backend-owned arc state.
