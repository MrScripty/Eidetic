...
- Partially resolved: timeline structural commands currently update backend-owned in-memory project state and trigger the existing save path. The node range and node lock commands now record sparse SQLite command/event/revision rows with replay protection; create, delete, split, child replacement, relationships, and notes still need the same history-backed treatment before claiming durable undo/redo or replay for all timeline edits.
