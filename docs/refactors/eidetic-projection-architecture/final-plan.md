...
- Partially resolved: timeline structural commands now record sparse SQLite command/event/revision rows with replay protection across node range, node create/delete/split, child replacement, node lock, node notes, and relationship create/delete commands, and all of those command handlers also persist SQLite current-state rows transactionally. Timeline render projections and read-only legacy timeline routes now reload the active SQLite project instead of reading the in-memory project mirror. Broad project save/autosave now preserves SQLite timeline current-state rows after timeline current state or revision history exists. Timeline command routes still update the in-memory project mirror and build command response projections from that mirror; move command response projections to canonical SQLite tables before claiming complete backend-owned timeline state.
