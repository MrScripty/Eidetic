...
- Partially resolved: story arc commands now own the HTTP mutation surface and record sparse SQLite command/event/revision rows with replay protection, but they still mirror into `Project.arcs` and rely on the full project save path for the current read model. Move arc projections and persistence to a canonical SQLite store before claiming complete backend-owned arc state.
...
- `crates/server/src/routes/commands.rs` and `crates/server/src/routes/commands_tests.rs` exceed the decomposition thresholds while owning many command handlers and route tests. Split timeline command handlers/tests into focused route modules before adding more semantic proposal or Bevy bridge command surfaces.
