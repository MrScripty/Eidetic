...
- `crates/server/src/history_store.rs` is over the decomposition threshold after adding read-side change review loading. Before adding undo/redo, semantic dependency traversal, or more review queries, split history schema/write recording from history read projections.
...
- Remaining: generate-children previews stay non-durable until the user applies the edited plan, and proposal-driven propagation review still needs semantic dependency projections plus proposal objects for downstream script/bible updates.
