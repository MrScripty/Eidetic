...
- Partially resolved: AI context preview, generation request construction, child generation request construction, and batch child discovery now read persisted timeline and story arc state from SQLite instead of the in-memory project mirror. AI generation status updates, generated-content status updates, and scene recap still mutate or read `Project.timeline` directly and need SQLite/proposal-backed treatment before command-route mirror mutation can be removed.
