...
- The cloned-project undo/redo routes still exist, but cloned snapshot producers have been removed. Replace or remove `/project/undo` and `/project/redo` when revision-backed undo/redo is introduced so the UI cannot depend on empty legacy history semantics.
