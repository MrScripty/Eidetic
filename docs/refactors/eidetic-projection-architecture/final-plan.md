...
- Partially resolved: timeline structural commands now record sparse SQLite command/event/revision rows with replay protection across node range, node create/delete/split, child replacement, node lock, node notes, and relationship create/delete commands; node-range, node-create, node-lock, node-notes, and relationship create/delete commands also persist SQLite current-state rows transactionally. Other timeline commands still update backend-owned in-memory project state and trigger the existing save path. Move the remaining timeline command current-state writes and timeline projections to canonical SQLite tables before claiming complete backend-owned timeline state.
