...
- Partially resolved: node-range commands no longer mutate the in-memory timeline mirror and validate against SQLite project state. The remaining timeline command routes still validate against and mutate the mirror; migrate them command-by-command to SQLite project inputs.
