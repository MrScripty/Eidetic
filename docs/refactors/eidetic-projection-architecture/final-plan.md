...
- Partially resolved: timeline structural commands now record sparse SQLite command/event/revision rows with replay protection across node range, node create/delete/split, child replacement, node lock, node notes, and relationship create/delete commands; node-range, node-create/delete/split, node-lock, node-notes, and relationship create/delete commands also persist SQLite current-state rows transactionally. Apply-children still updates backend-owned in-memory project state and triggers the existing save path. Move the remaining child-replacement current-state write and timeline projections to canonical SQLite tables before claiming complete backend-owned timeline state.
