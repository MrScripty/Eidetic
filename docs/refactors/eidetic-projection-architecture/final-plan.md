...
- `cargo check -p eidetic-server` also reports non-test dead-code warnings in `history_store.rs` (`RevisionOperation` import and `load_command`) because command replay helpers are currently used only by tests. Move test-only helpers behind `#[cfg(test)]` or add a production replay/read path before enabling warning-free gates.
