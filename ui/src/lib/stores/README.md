...
| `graphRendererCommandDrain.ts`            | Transient command orchestration     | Owns the polling lifecycle that drains validated Bevy graph renderer interaction commands.                                                | Keep as renderer-command bridge only; do not persist renderer state or durable graph facts here.              |
