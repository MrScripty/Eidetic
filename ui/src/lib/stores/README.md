...
| `graphRendererCommandDrain.ts`            | Polling owner for transient Bevy graph renderer command drain.                       |
| `graphRendererCommands.ts`                | Applies validated transient Bevy graph renderer commands to UI selection state.      |
| `graphRendererWindow.svelte.ts`           | Transient renderer-window status used to gate temporary command draining.            |
...
| `graphRendererCommandDrain.ts`            | Transient command orchestration     | Owns the polling lifecycle that drains validated Bevy graph renderer interaction commands only while renderer-window status says the scene is open and ready. | Keep as renderer-command bridge only; do not persist renderer state or durable graph facts here.              |
...
| `graphRendererWindow.svelte.ts`           | Transient renderer status           | Stores last-known renderer-window status so temporary command-drain polling is gated by backend-projected lifecycle state.                | Keep temporary until native renderer events replace polling; do not derive durable graph state here.          |
