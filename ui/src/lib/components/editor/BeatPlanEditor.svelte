    ...
    {#each editableBeats as beat, i}
      <div class="beat-item">
        <div class="beat-item-header">
          ...
          />
          <select class="beat-type-select" bind:value={beat.beat_type}>
            {#each beatTypes as bt}
              <option value={bt}>{beatTypeLabel(bt)}</option>
            {/each}
          </select>
          <button class="beat-remove-btn" title="Remove beat" onclick={() => removeBeat(i)}
            >&times;</button
          >
        </div>
        <textarea
              ...
              <div class="tag-list">
                {#each beat.characters ?? [] as char, ci}
                  <span class="tag character-tag">
                    {char}
                    <button class="tag-remove" onclick={() => removeTag(i, 'characters', ci)}
                      >&times;</button
                    >
                  </span>
                {/each}
              ...
              <div class="tag-list">
                {#each beat.props ?? [] as prop, pi}
                  <span class="tag prop-tag">
                    {prop}
                    <button class="tag-remove" onclick={() => removeTag(i, 'props', pi)}
                      >&times;</button
                    >
                  </span>
                {/each}
