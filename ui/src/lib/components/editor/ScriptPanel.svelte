...
<div class="script-panel" bind:this={scrollContainer}>
  <div class="script-panel-header">
    ...
    <span class="script-panel-count"
      >{sortedNodes.filter((n) => scriptText(n)).length} / {sortedNodes.length} beats</span
    >
  </div>
  ...
  <div class="script-panel-body">
    ...
    {:else}
      {#each sortedNodes as node (node.id)}
        ...
        {@const streamText =
          editorState.streamingNodeId === node.id && editorState.streamingText
        ...
        >
          <div class="beat-header">
            <span class="arc-dot" style="background: {arcColorForNode(node.id)}"></span>
            <span class="beat-name">{node.name}</span>
            {#if node.beat_type}
              <span class="beat-type"
                >{typeof node.beat_type === 'string' ? node.beat_type : node.beat_type.Custom}</span
              >
            {/if}
          </div>
          {#if text}
