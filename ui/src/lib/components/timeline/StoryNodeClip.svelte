<script lang="ts">
  ...
  let {
    ...
    compact = false,
    leftBoundMs = 0,
    rightBoundMs = TIMELINE.DURATION_MS,
    onselect,
    onmove,
    onresize,
    ondelete,
  ...
  function statusColor(status: ContentStatus): string {
    switch (status) {
      ...
      case 'NotesOnly':
        return 'var(--color-status-notes)';
      case 'Generating':
        return 'var(--color-status-generating)';
      case 'HasContent':
        return 'var(--color-status-generated)';
      default:
        return 'var(--color-text-muted)';
    }
  }
...
>
  <!-- Left resize handle -->
  <!-- svelte-ignore a11y_no_static_element_interactions -->
  <div class="resize-handle left" onpointerdown={(e) => handleResizeStart(e, 'left')}></div>
  ...
  {#if !compact}
    <!-- Connection handle -->
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div class="connect-handle" onpointerdown={handleConnectStart}></div>
  {/if}
...
<style>
  .node-clip {
    ...
    user-select: none;
    box-shadow:
      inset 1px 0 0 var(--color-overlay-medium),
      inset -1px 0 0 var(--color-overlay-medium);
  }
