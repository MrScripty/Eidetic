<script lang="ts">
  ...
  const svgWidth = $derived(nodes.length > 0 ? Math.max(...nodes.map((n) => n.x)) + 80 : 200);
  const svgHeight = $derived(nodes.length > 0 ? Math.max(...nodes.map((n) => n.y)) + 80 : 200);
...
<svg class="graph-svg" viewBox="0 0 {svgWidth} {svgHeight}" xmlns="http://www.w3.org/2000/svg">
  <!-- Edges -->
