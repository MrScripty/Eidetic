<script lang="ts">
  ...
  let {
    ...
    onclose,
    compact = false,
  }: { width: number; onclose: () => void; compact?: boolean } = $props();
  ...
  const entityMap = $derived(new Map(storyState.entities.map((e) => [e.id, e])));
  ...
  const selectedEntity = $derived(selectedNodeId ? (entityMap.get(selectedNodeId) ?? null) : null);
