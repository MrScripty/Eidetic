<script lang="ts">
  ...
  const templates = [
    {
      ...
      desc: 'Traditional multi-camera format with fast A/B cutting',
    },
    {
      id: 'single_cam',
      label: 'Single-Cam Dramedy',
      desc: 'Single-camera comedy-drama with cinematic pacing',
    },
    {
      id: 'animated',
      label: 'Animated Comedy',
      desc: 'Animated format with flexible scene structure',
    },
  ];
  ...
  async function handleNewProject(templateId: string) {
    ...
    try {
      ...
      notify(
        'error',
        `Failed to create project: ${e instanceof Error ? e.message : 'unknown error'}`,
      );
      busy = false;
    }
  }
