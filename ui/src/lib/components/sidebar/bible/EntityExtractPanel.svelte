<script lang="ts">
  ...
  const hasAny = $derived(result.new_entities.length > 0 || result.snapshot_suggestions.length > 0);

  const anySelected = $derived(
    entityAccepted.some(Boolean) ||
      snapshotAccepted.some(Boolean) ||
      result.entities_present.length > 0,
  );
</script>
...
<div class="extract-panel">
  ...
  {:else}
      ...
      <div class="section">
        ...
        {#each result.new_entities as sug, i}
          <label class="suggestion-row">
            ...
            <span class="sug-badge" style="color: {categoryColor(sug.category)}"
              >{sug.category}</span
            >
    ...
    <button class="apply-btn" disabled={!anySelected || applying} onclick={handleApply}>
