...
|-------------|-------------|
| `StoryBibleTab.svelte` | Top-level story-bible panel. |
...
## Constraints
- Entity state must stay aligned with server validation and persistence semantics.
...
## Decision
Keep story-bible components together and defer the `EntityDetail.svelte` split until shared field groups and relation editing can be extracted without changing the current UI contract.
...
## Invariants
- Entity detail edits remain backed by server APIs, not local-only schema forks.
...
## Dependencies
**Internal:** `ui/src/lib/stores/story.svelte.ts`, `ui/src/lib/api.ts`, `ui/src/lib/types.ts`.
**External:** Svelte.
