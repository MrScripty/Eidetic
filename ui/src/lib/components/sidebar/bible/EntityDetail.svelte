<script lang="ts">
  ...
  async function handleAddRelation() {
    ...
    await addRelation(entity.id, {
      target_entity_id: newRelationTarget,
  ...
  const otherEntities = $derived(storyState.entities.filter((e) => e.id !== entity.id));
</script>

<div class="entity-detail">
  <div class="detail-header">
    <button class="back-btn" onclick={onback}>&times; Close</button>
    <span class="category-label" style="color: {categoryColor(entity.category)}"
    ...
    <div class="header-actions">
      <button
        ...
        onclick={handleToggleLock}>{entity.locked ? '&#128274;' : '&#128275;'}</button
      >
  ...
  <div class="detail-body">
    ...
    {#if entity.details.type === 'Character'}
      ...
      <textarea
        ...
        oninput={(e) =>
          handleDetails({ voice_notes: (e.target as HTMLTextAreaElement).value } as any)}
      ></textarea>
      ...
      <textarea
        ...
        oninput={(e) =>
          handleDetails({ audience_knowledge: (e.target as HTMLTextAreaElement).value } as any)}
      ></textarea>
      ...
      <input
        ...
        oninput={(e) =>
          handleDetails({
            traits: (e.target as HTMLInputElement).value
              .split(',')
              .map((t) => t.trim())
              .filter(Boolean),
          } as any)}
    ...
    {:else if entity.details.type === 'Location'}
      ...
      <input
        ...
        oninput={(e) =>
          handleDetails({ scene_heading_name: (e.target as HTMLInputElement).value } as any)}
      />
      ...
      <textarea
        ...
        oninput={(e) =>
          handleDetails({ atmosphere: (e.target as HTMLTextAreaElement).value } as any)}
      ></textarea>
    {:else if entity.details.type === 'Prop'}
      ...
      <textarea
        ...
        oninput={(e) =>
          handleDetails({ significance: (e.target as HTMLTextAreaElement).value } as any)}
      ></textarea>
    {:else if entity.details.type === 'Theme'}
      ...
      <textarea
        ...
        oninput={(e) =>
          handleDetails({ manifestation: (e.target as HTMLTextAreaElement).value } as any)}
      ></textarea>
    {:else if entity.details.type === 'Event'}
      <label class="field-label">
        ...
        <input
          ...
          onchange={(e) =>
            handleDetails({ is_backstory: (e.target as HTMLInputElement).checked } as any)}
        />
      </label>
    {/if}
    ...
    <div class="color-presets">
      {#each COLOR_PRESETS as rgb}
        <button
          type="button"
          class="color-swatch"
          class:active={entity.color.r === rgb[0] &&
            entity.color.g === rgb[1] &&
            entity.color.b === rgb[2]}
