<script lang="ts">
  ...
  const sortedSnapshots = $derived([...entity.snapshots].sort((a, b) => a.at_ms - b.at_ms));
...
<div class="dev-timeline">
  ...
  {#if sortedSnapshots.length === 0 && !adding}
    <p class="empty-state">
      No development points yet. Add one to track how this entity changes over the episode.
    </p>
  {/if}
  ...
  <div class="points-list">
    {#each sortedSnapshots as snapshot, i (snapshot.at_ms + '-' + i)}
      <div class="point-row">
        ...
        {/if}
        <DevelopmentPointCard {snapshot} idx={i} onupdate={handleUpdate} ondelete={handleDelete} />
      </div>
    {/each}
  </div>
</div>
